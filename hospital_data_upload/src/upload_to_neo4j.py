...
NODES = ["Hospital", "Patient", "Payer", "Physician", "Review", "Visit"]
...
def set_uniqueness_constraints(transaction, node):
    """
    Enforces each node to have a unique ID
    """
    ...
    """
    _ = transaction.run(query, {})


# decorator reruns the function if there are intermittent connection issues
...
def load_hospital_graph_from_csv():
    ...
    with driver.session(database="neo4j") as session:
        # The visits query is set up so that existing nodes get updated with
        # certain information  if they have already been matched through the MERGE operation.
        # This ensures that data in the database can be refreshed or kept up to date without
